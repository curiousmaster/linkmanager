...
if ($search !== '') {
    foreach ($pages as $p) {
        ...
        foreach ($pageSecs as $sec) {
            ...
            // filter tools on the search term
    ...
    $pages = array_filter($pages, function($p) use($searchResults) {
        return isset($searchResults[$p['title']]);
    });
}
...
if ($search !== '') {
    ...
    if ($view!=='tree' && $currentPage) {
        $pageTitle .= " – " .
          htmlspecialchars($currentPage['title'], ENT_QUOTES);
    }
}
?><!DOCTYPE html>
<html lang="en">
...
<div class="d-flex" style="min-height: calc(100vh - 64px);">
  ...
  <nav id="sidebar" class="bg-dark">
    <div class="list-group list-group-flush">
      <?php foreach ($pages as $p): ?>
        <a href="?page=<?= urlencode($p['title']) ?>
                  &view=<?= htmlspecialchars($view,ENT_QUOTES) ?>
                  <?= $search? '&search='.urlencode($search):'' ?>"
           class="list-group-item list-group-item-action
             <?= strcasecmp($p['title'],$currentPageTitle)===0 ? ' active' : '' ?>">
  ...
  <main class="flex-grow-1 p-3">
    <?php if ($search !== ''): ?>
      <h3 class="text-white">
        Search results for “<?= htmlspecialchars($search, ENT_QUOTES) ?>”
      </h3>
        ...
        <?php foreach ($sections as $sec): if (empty($sec['tools'])) continue; ?>
          <div class="col-12">
            <h4 class="mt-4 mb-2 text-white">
              <?= htmlspecialchars($sec['name'], ENT_QUOTES) ?>
            </h4>
          </div>
          <?php foreach ($sec['tools'] as $tool): ?>
            <div class="col-12 col-sm-6 col-md-4">
              <div class="card h-100 d-flex flex-row align-items-stretch"
                   style="<?= 
                     (!empty($tool['background'])
                       ? 'background:'.htmlspecialchars($tool['background'],ENT_QUOTES).';'
                       : ''
                     )
                     . (!empty($tool['color']) 
                       ? 'color:'.htmlspecialchars($tool['color'],ENT_QUOTES).';'
                       : ''
                     );
                   ?>">
      ...
      <?php foreach ($sections as $sec): if (empty($sec['tools'])) continue; ?>
        ...
        <?php if ($sec['description']): ?>
          <p class="text-secondary mb-3">
            <?= htmlspecialchars($sec['description'], ENT_QUOTES) ?>
          </p>
        <?php endif; ?>
        <div class="row
